...
namespace BTL_QLCuaHangBanQuanAo.Views
{
    public partial class Form_TheLoai : Form
    {
        ...
        {
            data = connection.ExecuteQuery("SELECT * FROM TheLoai");
        ...
        {
            dgvDanhSach.DataSource = connection.ExecuteQuery($"SELECT * FROM TheLoai WHERE MaLoai LIKE '%{txtTimKiem.Text}%' OR TenLoai LIKE N'%{txtTimKiem.Text}%'");
        }
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (btnThem.Enabled == true)
            {
                if (Check())
                {
                    DataTable dataMaLoai = connection.ExecuteQuery($"SELECT * FROM TheLoai WHERE MaLoai = '{txtMa.Text}'");
                    if (dataMaLoai.Rows.Count > 0)
                    {
                    ...
                    if (MessageBox.Show("Bạn có muốn thêm  không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        try
                        {
                            string querry = $"INSERT INTO TheLoai VALUES('{txtMa.Text}', N'{txtTen.Text}')";
            ...
            if (btnXoa.Enabled == true)
            {
                if (MessageBox.Show("Bạn có muốn xoá không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        string querry = $"DELETE FROM TheLoai WHERE MaLoai = '{ma}'";
                        connection.ExecuteNonQuery(querry);
                        LoadData();
            ...
            if (btnSua.Enabled == true)
            {
                if (Check())
                {
                    ...
                    if (MessageBox.Show("Bạn có muốn sửa không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            string ma = dgvDanhSach.CurrentRow.Cells[0].Value.ToString();
                            string querry = $"UPDATE TheLoai SET MaLoai='{txtMa.Text}', TenLoai =N'{txtTen.Text}' WHERE MaLoai ='{ma}'";
                            connection.ExecuteNonQuery(querry);
                            LoadData();
        ...
        {
            DataProvider.Instance.ExportFileExcel(dgvDanhSach, "Danh Sách Thể Loại");
        }
    }
}
