...
{
    public partial class Form_Co : Form
    {
        ...
        {
            data = connection.ExecuteQuery("SELECT * FROM Co");
            dgvDanhSach.DataSource = data;

        }
        private void Form_Co_Load(object sender, EventArgs e)
        {
            LoadData();
        }
        ...
        {
            dgvDanhSach.DataSource = connection.ExecuteQuery($"SELECT * FROM Co WHERE MaCo LIKE '%{txtTimKiem.Text}%' OR TenCo LIKE N'%{txtTimKiem.Text}%'");
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (btnThem.Enabled == true)
            {
                if (Check())
                {
                    DataTable dataMaCo = connection.ExecuteQuery($"SELECT * FROM Co WHERE MaCo = '{txtMa.Text}'");
                    ...
                    if (MessageBox.Show("Bạn có muốn thêm  không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        try
                        {
                            string querry = $"INSERT INTO Co VALUES('{txtMa.Text}', N'{txtTen.Text}')";
            ...
            if (btnXoa.Enabled == true)
            {
                if (MessageBox.Show("Bạn có muốn xoá không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        string querry = $"DELETE FROM Co WHERE MaCo = '{ma}'";
                        connection.ExecuteNonQuery(querry);
                        LoadData();
            ...
            if (btnSua.Enabled == true)
            {
                if (Check())
                {
                    ...
                    if (MessageBox.Show("Bạn có muốn sửa không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            string ma = dgvDanhSach.CurrentRow.Cells[0].Value.ToString();
                            string querry = $"UPDATE Co SET MaCo='{txtMa.Text}', TenCo =N'{txtTen.Text}' WHERE MaCo ='{ma}'";
                            connection.ExecuteNonQuery(querry);
                            LoadData();
        ...
        {
            DataProvider.Instance.ExportFileExcel(dgvDanhSach, "Danh Sách Kích Cỡ");
        }
    }
}
