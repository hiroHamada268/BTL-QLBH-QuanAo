...
namespace BTL_QLCuaHangBanQuanAo.Views
{
    ...
    public partial class Form_DoiTuong : Form
    {
        ...
        {
            data = connection.ExecuteQuery("SELECT * FROM DoiTuong");
        ...
        {
            dgvDanhSach.DataSource = connection.ExecuteQuery($"SELECT * FROM DoiTuong WHERE MaDoiTuong LIKE '%{txtTimKiem.Text}%' OR TenDoiTuong LIKE N'%{txtTimKiem.Text}%'");
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            if (btnThem.Enabled == true)
            {
                if (Check())
                {
                    DataTable dataMaCo = connection.ExecuteQuery($"SELECT * FROM DoiTuong WHERE MaDoiTuong = '{txtMa.Text}'");
                    ...
                    if (MessageBox.Show("Bạn có muốn thêm  không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        try
                        {
                            string querry = $"INSERT INTO DoiTuong VALUES('{txtMa.Text}', N'{txtTen.Text}')";
            ...
            if (btnXoa.Enabled == true)
            {
                if (MessageBox.Show("Bạn có muốn xoá không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        string querry = $"DELETE FROM DoiTuong WHERE MaDoiTuong = '{ma}'";
                        connection.ExecuteNonQuery(querry);
                        LoadData();
            ...
            if (btnSua.Enabled == true)
            {
                if (Check())
                {
                    ...
                    if (MessageBox.Show("Bạn có muốn sửa không?", "Thông báo!", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            string ma = dgvDanhSach.CurrentRow.Cells[0].Value.ToString();
                            string querry = $"UPDATE DoiTuong SET MaDoiTuong='{txtMa.Text}', TenDoiTuong =N'{txtTen.Text}' WHERE MaDoiTuong ='{ma}'";
                            connection.ExecuteNonQuery(querry);
                            LoadData();
        ...
        {
            DataProvider.Instance.ExportFileExcel(dgvDanhSach, "Danh Sách Đố Tượng");
        }
    }
}
